...
diamonds |> 
  ggplot(aes(x = price)) +
  geom_histogram(binwidth = 50) +
  coord_cartesian(ylim = c(0, 500), xlim = c(0, 2000))
...
#proportion of each cut within a color
diamonds |> 
  count(color, cut) |> 
  group_by(color) |> 
  mutate(prop = n / sum(n)) |> 
...
nycflights13::flights |> 
  ...
  ggplot(aes(x = month, y = dest, fill = mean_delay)) +
  geom_tile() 
```
...
smaller |> 
  ggplot(aes(x = carat)) +
  geom_freqpoly(aes(group = cut_number(carat, 5)))
```
